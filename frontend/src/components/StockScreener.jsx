...
import { Layers, TrendingUp, ShieldAlert, Award, RefreshCw, HelpCircle, CheckCircle } from 'lucide-react';
  ...
  if (loading && !data) {
    return (
      <div className="glass-panel" style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
        Scanning Nifty 500 High-Alpha Institutional Stocks & Calculating Success Probabilities...
      </div>
    );
  }
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <h2 style={{ fontSize: '1.45rem', fontWeight: 800, color: '#fff', marginTop: '0.35rem' }}>
              Nifty 500 High-Growth Institutional Screener
            </h2>
      ...
      <div className="glass-panel" style={{ padding: '1.75rem' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', textAlign: 'left' }}>
            ...
            <tbody>
              {stocks.map((stk, idx) => {
                const todayChange = stk.todayChangePct || +1.85;
                ...
                return (
                  <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td className="mono" style={{ padding: '0.85rem 1rem' }}>
                      <div style={{ fontWeight: 700, color: '#fff' }}>₹{stk.cmp.toLocaleString('en-IN')}</div>
                    ...
                    <td className="mono" style={{ padding: '0.85rem 1rem', fontWeight: 700, color: '#f87171' }}>
                      ₹{stk.stopLoss ? stk.stopLoss.toLocaleString('en-IN') : Math.round(stk.cmp * 0.92).toLocaleString('en-IN')}
                    </td>
                    <td className="mono" style={{ padding: '0.85rem 1rem', fontWeight: 700, color: '#34d399' }}>
                      ₹{stk.targetPrice ? stk.targetPrice.toLocaleString('en-IN') : Math.round(stk.cmp * 1.15).toLocaleString('en-IN')}
                    </td>
                    <td style={{ padding: '0.85rem 1rem' }}>
